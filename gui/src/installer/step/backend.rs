        ...
        match &mut self.step {
            ConnectionStep::EnterEmail { email } => match message {
                Message::SelectBackend(message::SelectBackend::EmailEdited(value)) => {
                    email.valid = value.is_empty() || email_address::EmailAddress::is_valid(&value);
                    email.value = value;
                }
                Message::SelectBackend(message::SelectBackend::RequestOTP) => {
                    if email.value.is_empty() {
                        ...
                        return Command::perform(
                            async move {
                                let config = client::get_service_config(network)
                                    .await
...
impl Step for ImportRemoteWallet {
    ...
    fn apply(&mut self, ctx: &mut Context) -> bool {
        ...
        ctx.hw_is_used = true;
        ctx.descriptor = self.descriptor.clone();
        ctx.remote_backend = self.backend.clone();
