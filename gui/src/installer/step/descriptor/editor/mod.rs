...
impl Step for DefineDescriptor {
    ...
    fn update(&mut self, hws: &mut HardwareWallets, message: Message) -> Command<Message> {
        ...
        match message {
            ...
            Message::DefineDescriptor(message::DefineDescriptor::TemplatePage(p)) => {
                if let DescriptorTemplate::Custom { page } = &mut self.descriptor_template {
                    *page = p;
                }
            }
    ...
    fn view<'a>(
        ...
        let content = match self.descriptor_template {
            ...
            DescriptorTemplate::Custom { page } => {
                if page == 0 {
                    view::editor::template::custom::custom_template_primary(
                        progress,
                        self.use_taproot,
                    ...
                    view::editor::template::custom::custom_template_recovery(
                        progress,
                        self.use_taproot,
                        &mut self.paths[1..]
                            .iter()
