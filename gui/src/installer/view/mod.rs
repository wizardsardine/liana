...
pub fn defined_threshold<'a>(threshold: (usize, usize)) -> Element<'a, message::DefinePath> {
    Button::new(
        Row::new()
            .spacing(10)
            .push((0..threshold.1).fold(Row::new(), |row, i| {
                if i < threshold.0 {
                    row.push(icon::round_key_icon().style(color::GREEN))
                } else {
    ...
    Container::new(
        Column::new()
            ...
            .push(if sequence != 0 {
                ...
                Row::new()
                    .push(p1_regular("Able to move the funds at any time.").style(color::GREY_3))
                    .padding(5)
            })
