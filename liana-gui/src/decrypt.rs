...
use liana_ui::{
    ...
    icon,
    widget::{modal, Button, Container, Element},
};
...
impl DecryptModal {
    pub fn new(bytes: Vec<u8>, network: Network) -> Self {
        ...
        let derivation_paths = if let Some(backup) =
            ...
            h.insert(DerivationPath::from_str("48'/0'/0'/2'").expect("hardcoded"));
            h.insert(DerivationPath::from_str("48'/1'/0'/2'").expect("hardcoded"));
            h
        };
        Self {
            network,
    ...
    fn fetch(
        ...
        println!("Emit fetch task for {fingerprint}");
        Task::perform(
            async move {
                println!(
                    "Fetch {} {} {:?}",
...
fn widget_signing_device(
    ...
    .spacing(10);
    Button::new(row)
        // FIXME: reuse widget_style() from #1768
        .style(liana_ui::theme::button::secondary)
        // FIXME: reuse BTN_W from #1768
        .width(400)
}
