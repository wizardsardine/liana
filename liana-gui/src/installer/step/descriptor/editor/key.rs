...
impl SelectKeySource {
    ...
    fn view_other_options(&self) -> Element<Message> {
        ...
        let icon = if collapsed {
            icon::collapsed_icon().style(liana_ui::theme::text::secondary)
        } else {
            icon::collapse_icon().style(liana_ui::theme::text::secondary)
        };

        let btn = Button::new(
            row![p1_bold("Other options"), icon]
                .align_y(Vertical::Center)
                .spacing(5),
        )
        .on_press(Self::route(SelectKeySourceMessage::Collapse(!collapsed)))
        ...
        let mut col = Column::new().push(btn).spacing(5).width(BTN_W);
        if collapsed {
            col = col
                .push_maybe(load_key)
    ...
    fn widget_paste(&self, paste_type: PasteType) -> Element<Message> {
        let label = match paste_type {
            ...
            PasteType::Token => "Enter a Safety Net token",
        };
