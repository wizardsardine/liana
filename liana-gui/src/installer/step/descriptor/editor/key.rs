...
use iced::widget::{column, container, pick_list, row, Button, Column, Space};
use iced::{clipboard, Alignment, Length, Subscription, Task};
...
use liana_ui::component::hw::Account;
use liana_ui::component::text::{h3, p1_bold, p1_regular};
use liana_ui::component::{button, card, tooltip};
use liana_ui::theme::Theme;
use liana_ui::widget::{Container, Row};
use liana_ui::{color, icon, theme};
use liana_ui::{component::form, widget::Element};

use crate::app::settings::ProviderKey;
use crate::app::state::export::ExportModal;
use crate::export::{ImportExportMessage, ImportExportType};
use crate::hw::{is_compatible_with_tapminiscript, UnsupportedReason};
use crate::installer::message;
use crate::installer::view::editor::example_xpub;
use crate::services::keys::api::KeyKind;
use crate::services::{self, keys};
use crate::{
...
const MODAL_WIDTH: u16 = 550;
const BTN_W: u16 = 400;
const BTN_H: u16 = 40;

fn widget_style(theme: &Theme, status: Status) -> Style {
    liana_ui::theme::button::secondary(theme, status)
}
...
pub struct PathData {
    ...
    pub cosigners_kind: Vec<KeyKind>,
}
...
impl SelectKeySource {
        ...
        self.keys
            ...
            .map(|(fg, (_, key))| {
                ...
                if let KeySource::Token(kind, _) = key.source {
                    if !self.actual_path.cosigners_kind.contains(&kind) {
                        available = false;
                    }
                }
    ...
    fn route(msg: SelectKeySourceMessage) -> Message {
        ...
        match key {
            Ok(k) => {
                ...
                if let Some(key_kind) = k.source.provider_key_kind() {
                    ...
                    } else if !self.actual_path.cosigners_kind.contains(&key_kind) {
    ...
    fn on_account(&mut self, index: ChildNumber) -> Task<Message> {
        ...
        match self.focus {
            ...
            Focus::GenerateHotKey => Task::done(Self::route(
                SelectKeySourceMessage::FetchFromHotSigner(index),
            )),
            _ => Task::none(),
        }
    }
        ...
        if !self.processing {
            match self.step {
                Step::Select => {
                    ...
                    if let SelectedKey::Existing(fg) = self.selected_key {
                        if let Some((_, key)) = self.keys.get(&fg) {
                            self.form_alias.value = key.name.clone();
                            self.form_alias.valid = true;
                        }
                    }
                }
    ...
    fn main_view(
        ...
        let only_token = !self.actual_path.cosigners_kind.is_empty();

        let no_devices = (hws.is_empty() && self.keys.is_empty() && !only_token)
            .then_some(self.view_no_devices());

        let devices = ((!hws.is_empty() || !self.keys.is_empty()) && !only_token)
            .then_some(self.view_signing_devices(&hws));

        let keys = (!self.keys.is_empty() && !only_token).then_some(self.view_keys());

        let header = Row::new()
            .push(h3("Select key source"))
            .push(Space::with_width(Length::Fill))
            .push(
                button::transparent(Some(icon::cross_icon().size(40)), "").on_press(Message::Close),
            )
    ...
    fn details_view(&self) -> Element<Message> {
        ...
        let header = row![
            button::transparent(Some(icon::arrow_back().size(25)), "")
                .on_press(Self::route(SelectKeySourceMessage::Previous)),
        ...
        let pick_enabled =
            !self.processing && matches!(self.focus, Focus::Device(_) | Focus::GenerateHotKey);
        ...
        let edit_account = matches!(self.focus, Focus::Device(_) | Focus::GenerateHotKey);
        ...
        let column = Column::new()
            ...
            .push(
                container(form::Form::new(
                    "E.g. My Hardware Wallet",
                    &self.form_alias,
    ...
    fn view_signing_devices(
        ...
        let mut col = column![p1_bold("Detected hardwares")]
            .spacing(5)
            .width(BTN_W);
        for hw in hws {
    ...
    fn view_other_options(&self) -> Element<Message> {
        let paste_token = (!self.actual_path.cosigners_kind.is_empty())
            .then_some(self.widget_paste(PasteType::Token));

        let collapsed = self.options_collapsed || paste_token.is_some();

        let btn = if collapsed {
            Button::new(icon::collapsed_icon().style(liana_ui::theme::text::secondary))
                .on_press(Self::route(SelectKeySourceMessage::Collapse(false)))
                .style(liana_ui::theme::button::transparent_border)
        } else {
            Button::new(icon::collapse_icon().style(liana_ui::theme::text::secondary))
                .on_press(Self::route(SelectKeySourceMessage::Collapse(true)))
                .style(liana_ui::theme::button::transparent_border)
        };
        ...
        let paste_xpub = paste_token
            .is_none()
            .then_some(self.widget_paste(PasteType::Xpub));

        let mut col = Column::new()
            .push(
                row![p1_bold("Other options"), btn]
                    .align_y(Vertical::Center)
                    .spacing(5),
            )
            .spacing(5)
    ...
    fn widget_signing_device(
        ...
        let (alias, fg, state, support_taproot) = device;
        let mut enabled = true;
        // TODO: move the message logic to self.detected_hws()
        ...
        let message = message.map(p1_regular);
        let designation = column![
            p1_bold(alias),
            p1_regular(fg.map(|fg| format!("#{fg}")).unwrap_or(" - ".to_string()))
        ]
        .align_x(Horizontal::Center);
        let row = Row::new()
        ...
        if enabled {
            if let Some(fg) = fg {
                btn = btn.on_press(Self::route(SelectKeySourceMessage::SelectDevice(*fg)))
            }
        }
        btn.into()
    }
    fn widget_key(
        ...
        let message = if let KeySource::Token(kind, _) = source {
            if !self.actual_path.cosigners_kind.contains(&kind) {
                Some("Token type not allowed in this path")
            } else {
                None
            }
        } else {
            (!enable).then_some("Key already used in this path")
        };
        let fg_str = format!("#{}", fg);
        let row = Row::new()
            .push(icon)
            .push(column![p1_bold(alias), p1_regular(fg_str)].align_x(Horizontal::Center))
            .push_maybe(message)
            .spacing(10)
            .align_y(Vertical::Center);
        let mut btn = Button::new(container(row)).style(widget_style).width(BTN_W);
        if message.is_none() {
            btn = btn.on_press(Self::route(SelectKeySourceMessage::SelectKey(fg)))
        }
        btn.into()
    }
    fn widget_load_key(&self) -> Element<Message> {
        let error = self.import_xpub_error.clone().map(|e| {
            row![
                p1_regular(e).color(color::ORANGE),
                Space::with_width(Length::Fill)
            ]
        });

        let row = row![icon::import_icon(), "Import extended public key file"]
            .spacing(10)
            .align_y(Vertical::Center)
            .height(BTN_H);
        ...
        if !matches!(self.focus, Focus::LoadXpubFromFile) {
            btn = btn.on_press(Self::route(SelectKeySourceMessage::SelectLoadXpub))
        }
        btn.into()
    }
    fn widget_generate_hot_key(&self) -> Element<Message> {
        let row = row![
            icon::round_key_icon().color(color::RED),
            "Generate hot keys stored on this computer",
        ...
        .align_y(Vertical::Center);
        let mut btn = Button::new(container(row)).style(widget_style).width(BTN_W);
        if !matches!(self.focus, Focus::GenerateHotKey) {
            btn = btn.on_press(Self::route(SelectKeySourceMessage::SelectGenerateHotKey))
        }
        btn.into()
    }
    fn widget_paste(&self, paste_type: PasteType) -> Element<Message> {
        let label = match paste_type {
            PasteType::Xpub => "Paste extended public key",
            PasteType::Token => "Enter cosigner token",
        };

        let form = match paste_type {
            PasteType::Xpub => {
                form::Form::new(&example_xpub(self.network), &self.form_xpub, |xpub| {
                    Self::route(SelectKeySourceMessage::Xpub(xpub))
                })
            }
            PasteType::Token => form::Form::new("aaaa-bbbb-cccc", &self.form_token, |token| {
                Self::route(SelectKeySourceMessage::Token(token))
            }),
        };

        let paste = Button::new(icon::clipboard_icon().style(liana_ui::theme::text::secondary))
            .on_press(Self::route(match paste_type {
                PasteType::Xpub => SelectKeySourceMessage::PasteXpub,
                PasteType::Token => SelectKeySourceMessage::PasteToken,
            }))
            .style(liana_ui::theme::button::transparent_border);

        let focus = match paste_type {
            PasteType::Xpub => Focus::EnterXpub,
            PasteType::Token => Focus::EnterToken,
        };

        let icon = match paste_type {
            PasteType::Xpub => icon::paste_icon(),
            PasteType::Token => icon::enter_box_icon(),
        }
        .color(color::WHITE);
        ...
        if self.focus == focus {
            let col = Column::new()
                .push(row![
                    p1_regular(label).color(color::WHITE),
                    Space::with_width(Length::Fill)
                ])
                .push(row![form, paste].spacing(10));

            Button::new(row![icon, col].align_y(Vertical::Center).spacing(10)).style(widget_style)
        } else {
            Button::new(
                row![icon, p1_regular(label)]
                    .height(BTN_H)
                    .spacing(10)
                    .align_y(Vertical::Center),
            )
            .on_press(Self::route(match paste_type {
                PasteType::Xpub => SelectKeySourceMessage::SelectEnterXpub,
                PasteType::Token => SelectKeySourceMessage::SelectEnterToken,
            }))
            .style(widget_style)
        }
        .width(BTN_W)
    ...
    fn update(&mut self, hws: &mut HardwareWallets, message: Message) -> Task<Message> {
        ...
        match message {
            ...
            Message::SelectKeySource(sksm) => match sksm {
                ...
                SelectKeySourceMessage::FetchProvider(_) => todo!(),
