    ...
    fn redraft(&mut self, daemon: Arc<dyn Daemon + Sync + Send>) {
        ...
        match tokio::runtime::Handle::current().block_on(async {
            ...
            Ok(CreateSpendResult::Success { psbt, .. }) => {
                // HACK: It's a bad hack, here create_spend_tx() success while
                // it should not (MAX amount == 0)
                let max_under_dust = psbt.outputs.len() != total_recipients;
                ...
                if !max_under_dust {
                    ...
                    if let Some((i, recipient)) = recipient_with_max {
                        ...
                        let amount = psbt
                            ...
                            .find(|o| {
                                o.script_pubkey
                                    == max_address.clone().assume_checked().script_pubkey()
                            })
                    ...
                    // HACK: It's a bad hack, here create_spend_tx() has success while
                    // it should not (MAX amount == 0)

                    handle_max_under_dust(
                        &mut self.fee_amount,
...
fn handle_max_under_dust(
    ...
    *fee_amount = None;
    ...
    if let Some((i, recipient)) = recipient_with_max {
        *amount_left_to_select = None;
        recipient.estimated_max_amount = Some(Amount::from_sat(DUST_OUTPUT_SATS - missing));
...
impl Step for DefineSpend {
                    ...
                    view::CreateSpendMessage::SendMaxToRecipient(i) => {
                        if let Some(recipient) = self.recipients.get_mut(i) {
                            if self.send_max_to_recipient == Some(i) {
                                ...
                                recipient.estimated_max_amount = None;
                            } else {
    ...
    fn view<'a>(&'a self, cache: &'a Cache) -> Element<'a, view::Message> {
        ...
        let max_under_dust = self
            .recipients
            .iter()
            .any(|r| r.estimated_max_amount.is_some());
...
struct Recipient {
    ...
    estimated_max_amount: Option<Amount>,
