...
pub mod global {
    ...
    impl GlobalSettings {
        pub fn datadir(global_datadir: &LianaDirectory) -> PathBuf {
        ...
        pub fn update_window_config(
            ...
            let mut cfg = Self::load(path)
                .map_err(app::Error::Config)?
                .unwrap_or_default();
            cfg.window_config = Some(window_config);
            cfg.to_file(path).map_err(app::Error::Config)
        }
    ...
    impl PersistedBitboxNoiseConfig {
        ...
        pub fn new(global_datadir: &LianaDirectory) -> PersistedBitboxNoiseConfig {
            PersistedBitboxNoiseConfig {
                file_path: GlobalSettings::datadir(global_datadir),
            }
        }
    }
