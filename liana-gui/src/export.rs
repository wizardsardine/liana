...
const DUMP_LABELS_LIMIT: u32 = 200;
    ...
    iced::stream::channel(100, move |mut output| async move {
        ...
        loop {
            ...
            let handle = match state.handle.take() {
                Some(h) => h,
                None => {
                    let _ = output.send(Progress::Error(Error::HandleLost)).await;
                    continue;
                }
            };
            let msg = {
            ...
            sleep(time::Duration::from_millis(100)).await;
            let _ = output.send(Progress::None).await;
        }
    })
}
...
pub async fn export_labels(
    ...
    let mut offset = 0u32;
    loop {
        let mut fetched = match daemon.dump_labels(offset, DUMP_LABELS_LIMIT).await {
            Ok(l) => l,
            Err(e) => {
                send_error!(sender, e.into());
                return;
