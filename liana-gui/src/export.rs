...
pub async fn export_subscription(mut state: Export) -> (Progress, Export) {
    match state.state() {
        Status::Init => {
            state.start().await;
        }
    ...
    let handle = match state.handle.take() {
        Some(h) => h,
        None => return (Progress::Error(Error::HandleLost), state),
    };
    {
        let h = handle.lock().expect("should not fail");
        if h.is_finished() {
            return (Progress::Finished, state);
        } else if disconnected {
            return (Progress::Error(Error::ChannelLost), state);
        }
    } // => release handle lock
