...
pub async fn import_backup(
    ...
    let backup_labels = if let Some(labels) = account.labels.clone().map(|l| l.into_vec()) {
        let db_labels: BTreeSet<_> = match daemon.get_labels_bip329(0, u32::MAX).await {
            Ok(l) => l.into_vec(),
            Err(_) => {
        ...
        for l in &labels {
            if db_labels.contains(l) {
                send_error!(sender, Error::LabelsConflict(ack_sender));
                conflict = true;
                break;
            }
        }
    ...
    // check if key aliases can be imported
    ...
    let settings = if !account.keys.is_empty() {
        ...
        let (ack_sender, ack_receiver) = sync_channel(0);
        let mut conflict = false;
        for fg in account.keys.keys() {
            // if there is a conflict, we ask the user to ACK before overwriting
            if settings_aliases.contains_key(fg) {
                send_error!(sender, Error::KeyAliasesConflict(ack_sender));
                conflict = true;
                break;
            }
        }
