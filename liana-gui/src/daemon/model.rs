...
use liana::descriptors::{LianaDescError, LianaDescriptor};
pub use liana::{
...
/// Build a filtered PSBT for use with `partial_spend_info`, stripping any non-Liana inputs
/// added by a payjoin receiver. Returns the original PSBT unchanged for non-payjoin cases.
fn payjoin_filtered_spend_info(
    desc: &LianaDescriptor,
    psbt: &Psbt,
    coins: &HashMap<OutPoint, Coin>,
    payjoin_status: &Option<PayjoinStatus>,
) -> Result<PartialSpendInfo, LianaDescError> {
    if coins.len() != psbt.inputs.len() && payjoin_status.is_some() {
        let liana_indices: Vec<usize> = psbt
            .unsigned_tx
            .input
            .iter()
            .enumerate()
            .filter(|(_, txin)| coins.contains_key(&txin.previous_output))
            .map(|(i, _)| i)
            .collect();
        ...
        filtered.unsigned_tx.input = liana_indices
            ...
            .collect();
        desc.partial_spend_info(&filtered)
    } else {
...
impl SpendTx {
    pub fn new(
        ...
        let sigs = payjoin_filtered_spend_info(desc, &psbt, &coins_map, &payjoin_status)
    ...
    /// Compute partial spend info for this transaction's PSBT, filtering out any
    /// non-Liana inputs added by a payjoin receiver before calling the descriptor.
    pub fn partial_spend_info(
        &self,
        desc: &LianaDescriptor,
    ) -> Result<PartialSpendInfo, LianaDescError> {
        payjoin_filtered_spend_info(desc, &self.psbt, &self.coins, &self.payjoin_status)
    }
