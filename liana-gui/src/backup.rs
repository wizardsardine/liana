...
impl Backup {
    pub async fn from_installer(ctx: &Context, timestamp: bool) -> Result<Self, Error> {
        ...
        let name = if let Some(settings) = settings {
            ...
            settings.keys.iter().for_each(|k| {
                let key = Key {
                    key: k.master_fingerprint,
                    alias: Some(k.name.clone()),
                    role: None,
                    key_type: None,
                };
                account.keys.insert(k.master_fingerprint, key);
            });
    ...
    pub async fn from_app(
        ...
        let key_aliases = &wallet.keys_aliases;
        ...
        // TODO: get derivation indexes
        // TODO: timestamp?
        for (fg, alias) in key_aliases {
            let key = Key {
                key: *fg,
            ...
            account.keys.insert(*fg, key);
        }

        account.labels = Some(daemon.dump_labels(0, u32::MAX).await?);
