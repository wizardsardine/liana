...
fn psbt_row(row: PsbtRow) -> Element<'static, DebugMessage> {
    ...
    let status_pill: Option<Container<'static, DebugMessage>> = match row.status {
        Status::Pending => None,
        Status::Broadcast => Some(pill::unconfirmed().width(120.0)),
        Status::Spent => Some(pill::spent().width(120.0)),
        Status::Deprecated => Some(pill::deprecated().width(120.0)),
    };
    ...
    let amount_col: Column<'static, DebugMessage> = Column::new()
        ...
        .push_maybe(
            row.fee_sats
                .map(|f| amount_with_font(&Amount::from_sat(f), P2_REGULAR_SPEC)),
        )
    ...
    let body = Row::new()
        ...
        .push_maybe(row.is_batch.then(pill::batch))
