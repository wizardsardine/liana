    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Self::IncompatibleDesc => write!(
                f,
                "Descriptor is not compatible with a Coincube spending policy."
            ),
...
impl PathInfo {
    ...
    /// Returns None if the policy does not describe the primary spending path of a Coincube
    ...
    /// Returns None if the policy does not describe the recovery spending path of a Coincube
...
/// A Coincube spending policy is one composed of at least two spending paths:
...
/// A Coincube policy can be created from some settings (the primary and recovery keys, the
...
/// **cannot roundtrip** a descriptor through a `CoincubePolicy`.
#[derive(Debug, Eq, PartialEq, Clone)]
pub struct CoincubePolicy {
...
impl CoincubePolicy {
    /// Create a new Coincube policy from a given configuration.
    ///
    ...
    /// Create a new Coincube policy for use under a Taproot context.
    pub fn new(
        primary_path: PathInfo,
        recovery_paths: BTreeMap<u16, PathInfo>,
    ) -> Result<CoincubePolicy, CoincubePolicyError> {
    ...
    /// Create a new Coincube policy for use under a P2WSH context.
    pub fn new_legacy(
        primary_path: PathInfo,
        recovery_paths: BTreeMap<u16, PathInfo>,
    ) -> Result<CoincubePolicy, CoincubePolicyError> {
    ...
    /// Create a Coincube policy from a descriptor. This will check the descriptor is correctly formed
    ...
    /// are inferring a `CoincubePolicy` from a descriptor, generating a descriptor from this
    /// `CoincubePolicy` may not yield the same descriptor.
    pub fn compile_multipath_descriptor(
        ...
        self.compile_multipath_descriptor_fallible()
            .expect("This is always checked when creating a CoincubePolicy.")
    }
...
/// Information about a partial spend of Coincube coins
#[derive(Debug, Eq, PartialEq, Clone)]
