...
fn sanity_check_psbt(
    spent_desc: &descriptors::CoincubeDescriptor,
    psbt: &Psbt,
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            CreateSpendWarning::ChangeAddedToFee(amt) => write!(
                f,
                "Dust UTXO. The minimal change output allowed by Coincube is {} sats. \
...
/// * `main_descriptor`: the multipath Coincube descriptor, used to derive the addresses of the
///   candidate coins.
