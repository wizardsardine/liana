...
// The minimum bitcoind version that can be used with coincubed.
...
impl BitcoinD {
    ...
    fn rebuild_client(&self, kind: ClientKind) -> Result<Client, BitcoindError> {
        let node_url = format!("http://{}", self.config.addr);
        let watchonly_url = format!(
            "http://{}/wallet/{}",
            self.config.addr, self.watchonly_wallet_path
        );
        let builder = match &self.config.rpc_auth {
            config::BitcoindRpcAuth::CookieFile(cookie_path) => {
                let cookie_string =
                    fs::read_to_string(cookie_path).map_err(BitcoindError::CookieFile)?;
                MinreqHttpTransport::builder().cookie_auth(cookie_string)
    ...
    fn make_request_inner(
        ...
        match attempt(self.client(kind)) {
    ...
    fn import_descriptor(&self, desc: &CoincubeDescriptor) -> Option<String> {
...
mod tests {
    ...
    fn test_current_descs_contain_desc_timestamp() {
        ...
        // Create the Vault wallet descriptor:
        let desc = CoincubeDescriptor::from_str("tr([1dce71b2/48'/1'/0'/2']tpubDEeP3GefjqbaDTTaVAF5JkXWhoFxFDXQ9KuhVrMBViFXXNR2B3Lvme2d2AoyiKfzRFZChq2AGMNbU1qTbkBMfNv7WGVXLt2pnYXY87gXqcs/<0;1>/*,and_v(v:pk([1dce71b2/48'/1'/0'/2']tpubDEeP3GefjqbaDTTaVAF5JkXWhoFxFDXQ9KuhVrMBViFXXNR2B3Lvme2d2AoyiKfzRFZChq2AGMNbU1qTbkBMfNv7WGVXLt2pnYXY87gXqcs/<2;3>/*),older(65535)))").unwrap();
