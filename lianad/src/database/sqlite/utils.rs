...
fn migrate_v8_to_v9(conn: &mut rusqlite::Connection) -> Result<(), SqliteDbError> {
    db_exec(conn, |db_tx| {
        db_tx.execute(
            "ALTER TABLE payjoin_receivers ADD COLUMN derivation_index INTEGER",
            [],
        )?;
        db_tx.execute("UPDATE version SET version = 9", [])?;
        Ok(())
    })?;
...
fn migrate_v9_to_v10(conn: &mut rusqlite::Connection) -> Result<(), SqliteDbError> {
    db_exec(conn, |db_tx| {
        db_tx.execute("ALTER TABLE payjoin_receivers ADD COLUMN bip21 TEXT", [])?;
        db_tx.execute("UPDATE version SET version = 10", [])?;
        Ok(())
    })?;
