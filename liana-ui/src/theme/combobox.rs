...
pub fn input(theme: &Theme, status: InputStatus) -> InputStyle {
    ...
    let icon = match status {
        InputStatus::Active | InputStatus::Disabled => theme.colors.buttons.pick_list.active.text,
        InputStatus::Hovered | InputStatus::Focused { .. } => {
            theme.colors.buttons.pick_list.hovered.text
        }
    };
