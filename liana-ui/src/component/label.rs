...
{
    // An empty label is not an error (no warning shown), but a new address
    // cannot be generated without one, so the button stays disabled.
    let confirm = (value.valid && (!new || !value.value.is_empty())).then_some(confirm);
