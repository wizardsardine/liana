        ...
        Task::perform(
            async move {
                ...
                let error = match (&info, &payments) {
                    (Err(e1), Err(e2)) => {
                        Some(view::LiquidOverviewError::BalanceAndTransactionsFetch(
                            e1.to_string(),
                            e2.to_string(),
                        ))
                    }
                    (Err(e), _) => Some(view::LiquidOverviewError::BalanceFetch(e.to_string())),
                    (_, Err(e)) => {
                        Some(view::LiquidOverviewError::TransactionsFetch(e.to_string()))
                    }
                    _ => None,
                };
