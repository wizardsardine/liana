        ...
        match message {
            Message::View(view::Message::Home(msg)) => match msg {
                            ...
                            match direction {
                                TransferDirection::ActiveToVault => {
                                    ...
                                    if let Some((min_sat, max_sat)) = self.onchain_send_limit {
                                        if entered_sat < min_sat || entered_sat > max_sat {
                                            valid = false;
                                            warning = Some("Amount outside onchain send limits");
                                        }
                                    }
                                }
                                TransferDirection::VaultToActive => {
                                    ...
                                    if let Some((min_sat, max_sat)) = self.onchain_receive_limit {
                                        if entered_sat < min_sat || entered_sat > max_sat {
                                            valid = false;
                                            warning = Some("Amount outside onchain receive limits");
                                        }
                                    }
                                }
                            }
                                        ...
                                        async move {
                                            breez_client
                                                .pay_onchain(&PayOnchainRequest {
                                                    ...
                                                    prepare_response: prepare_onchain_send_response,
                                                })
                                                .await
                                        },
                                        |result| match result {
                            ...
                            if let Some(address_info) = self.receive_address_info.clone() {
                                if let Some(daemon) = daemon {
                                    ...
                                    if let Ok(amount) = Amount::from_str_in(
                                        ...
                                        return Task::perform(
                                            async move {
                                                let feerate_vb = FeeEstimator::new().get_high_priority_rate().await.unwrap();
                ...
                HomeMessage::BreezOnchainAddress(address) => {
                    let address = address.split_once(":").unwrap().1.split_once("?").unwrap().0;
                ...
                HomeMessage::RefreshActiveBalance => {
