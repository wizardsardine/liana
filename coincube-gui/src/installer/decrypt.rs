        ...
        .decrypt()
    {
        Ok(dec) => match dec {
            Decrypted::Descriptor(d) => {
                let descr = match CoincubeDescriptor::from_str(&d.to_string()) {
                    Ok(descr) => descr,
            ...
            Decrypted::WalletBackup(backup_bytes) => {
                ...
                let _ = match CoincubeDescriptor::from_str(descriptor_str) {
                    Ok(descr) => descr,
                    Err(_) => {
        ...
        match msg {
            ...
            Decrypt::UnexpectedPayload(p) => match p {
                Decrypted::Descriptor(_) => {
                    tracing::error!("Descriptor decrypted but not a valid coincube descriptor");
    ...
    let content = match state.error {
        Some(e) => match e {
            ...
            Error::InvalidDescriptor => invalid_content(
                "The file have been decrypted but the descriptor is not a valid Coincube descriptor.",
            ),
        },
