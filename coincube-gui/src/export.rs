...
use coincube_core::{
    descriptors::{bip341_nums, CoincubeDescriptor},
    miniscript::{
...
pub async fn import_descriptor(
    ...
    let descriptor = if let Ok(d) = CoincubeDescriptor::from_str(descr_str) {
        Some(d)
    } else {
        let backup: Result<Backup, _> = serde_json::from_str(descr_str);
        backup.ok().and_then(|b| {
            if b.accounts.len() == 1 {
                CoincubeDescriptor::from_str(&b.accounts[0].descriptor).ok()
            } else {
                None
            }
        })
...
pub async fn import_backup(
    ...
    let backup_descriptor = match CoincubeDescriptor::from_str(&account.descriptor) {
        Ok(d) => d,
        Err(_) => {
            return Err(Error::BackupImport(
                "The backup descriptor is not a valid Coincube descriptor!".into(),
            ));
        }
    };
...
pub async fn from_backup(sender: &UnboundedSender<Progress>, path: PathBuf) -> Result<(), Error> {
    ...
    let backup = match backup {
        ...
        Err(e) => {
            ...
            let descr = CoincubeDescriptor::from_str(&backup_str);
            match descr {
                Ok(descr) => {
    ...
    let descriptor = match CoincubeDescriptor::from_str(&account.descriptor) {
        Ok(d) => d,
        Err(_) => {
            return Err(Error::BackupImport(
                "The backup descriptor is not a valid Coincube descriptor!".into(),
...
> {
    ...
    let backup_descriptor = CoincubeDescriptor::from_str(&account.descriptor)
...
mod tests {
    ...
    async fn test_import_descriptor_from_file() {
        ...
        let descr = CoincubeDescriptor::from_str(raw_descriptor).unwrap();
        let _msg = receiver.try_recv().unwrap();
    ...
    async fn test_import_descriptor_from_backup_file() {
        ...
        let path = env::current_dir()
            .unwrap()
            .join("test_assets")
            .join("coincube-backup-2025-06-23T13-23-54.json");
        println!("path: {}", path.display());
        import_descriptor(&sender, path).await.unwrap();
        let _msg = receiver.try_recv().unwrap();
        assert!(matches!(Progress::Progress(100.0), _msg));
