...
def additional_fees(anc_vsize, anc_fee, target_feerate):
    """The additional fee which must have been computed by coincubed."""
...
def test_spend_change(coincubed, bitcoind):
    ...
    destinations = {
        ...
        coincubed.rpc.getnewaddress()["address"]: 100_000,
    }
...
def test_coin_marked_spent(coincubed, bitcoind):
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: int(0.04 * COIN / 2),
    }
    res = coincubed.rpc.createspend(destinations, [outpoint_4], 18)
    psbt = PSBT.from_base64(res["psbt"])
    sign_and_broadcast(psbt)
    assert len(psbt.o) == 2
    assert len(res["warnings"]) == 0
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: int(0.01 * COIN),
        coincubed.rpc.getnewaddress()["address"]: int(0.01 * COIN),
        bitcoind.rpc.getnewaddress(): int(0.01 * COIN),
    }
    res = coincubed.rpc.createspend(destinations, [outpoint_5, outpoint_6], 2)
    psbt = PSBT.from_base64(res["psbt"])
    sign_and_broadcast(psbt)
    assert len(psbt.o) == 4
...
def test_send_to_self(coincubed, bitcoind):
    ...
    res = coincubed.rpc.createspend({}, outpoints, specified_feerate)
    spend_psbt = PSBT.from_base64(res["psbt"])
...
def test_coin_selection(coincubed, bitcoind):
    ...
    assert "missing" in coincubed.rpc.createspend({dest_addr_1: 100_000}, [], 2)

    # Receive a coin in an unconfirmed deposit transaction.
    recv_addr_1 = coincubed.rpc.getnewaddress()["address"]
    ...
    assert len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 0
    assert len(coincubed.rpc.listcoins(["unconfirmed"])["coins"]) == 1
    assert coincubed.rpc.listcoins(["unconfirmed"])["coins"][0]["is_change"] is False
    assert "missing" in coincubed.rpc.createspend({dest_addr_1: 100_000}, [], 2)
    ...
    spend_res_1 = coincubed.rpc.createspend({dest_addr_1: 100_000}, [], 2)
    ...
    assert "missing" in coincubed.rpc.createspend({dest_addr_1: 200_000}, [], 2)
    ...
    assert len(coincubed.rpc.listcoins(["unconfirmed"])["coins"]) == 1
    assert coincubed.rpc.listcoins(["unconfirmed"])["coins"][0]["is_change"] is True
    assert coincubed.rpc.listcoins(["unconfirmed"])["coins"][0]["is_from_self"] is True
    ...
    # Try 10 sat/vb:
    feerate = 10
    spend_res_2 = coincubed.rpc.createspend({dest_addr_2: 10_000}, [], feerate)
    ...
    recv_addr_2 = coincubed.rpc.listaddresses(10, 1)["addresses"][0]["change"]
    deposit_2 = bitcoind.rpc.sendtoaddress(recv_addr_2, 30_000 / COIN)
    wait_for(lambda: len(coincubed.rpc.listcoins(["unconfirmed"])["coins"]) == 2)
    assert (
        len(
            [
                c
                for c in coincubed.rpc.listcoins(["unconfirmed"])["coins"]
                if c["is_from_self"]
            ]
        )
        == 1
    ...
    assert "missing" in coincubed.rpc.createspend({dest_addr_3: 20_000}, [], 10)
    ...
    outpoints = [c["outpoint"] for c in coincubed.rpc.listcoins(["unconfirmed"])["coins"]]

    feerate = 10
    spend_res_3 = coincubed.rpc.createspend({dest_addr_3: 20_000}, outpoints, feerate)
    ...
    spend_txid_3 = sign_and_broadcast_psbt(coincubed, spend_psbt_3)
    ...
    spend_res_4 = coincubed.rpc.createspend({dest_addr_4: 15_000}, [], 2)
    ...
    res_manual = coincubed.rpc.createspend({dest_addr_4: 15_000}, outpoints, 2)
...
def test_coin_selection_changeless(coincubed, bitcoind):
    """We choose the changeless solution with lowest fee."""
    # Get two coins with similar amounts.
    txid_a = bitcoind.rpc.sendtoaddress(coincubed.rpc.getnewaddress()["address"], 0.00031)
    txid_b = bitcoind.rpc.sendtoaddress(coincubed.rpc.getnewaddress()["address"], 0.00032)
    bitcoind.generate_block(1, wait_for_mempool=[txid_a, txid_b])
    wait_for(lambda: len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 2)
    # Send an amount that can be paid by just one of our coins.
    res = coincubed.rpc.createspend({bitcoind.rpc.getnewaddress(): 30800}, [], 1)
    psbt = PSBT.from_base64(res["psbt"])
...
def test_sweep(coincubed, bitcoind):
    """
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: 0.8,
        coincubed.rpc.getnewaddress()["address"]: 0.12,
        coincubed.rpc.getnewaddress()["address"]: 1.87634,
        coincubed.rpc.getnewaddress()["address"]: 1.124,
    }
    ...
    # sweep address.
    all_coins = coincubed.rpc.listcoins()["coins"]
    balance = sum(c["amount"] for c in all_coins)
    all_outpoints = [c["outpoint"] for c in all_coins]
    destinations = {}
    change_addr = bitcoind.rpc.getnewaddress()
    res = coincubed.rpc.createspend(destinations, all_outpoints, 1, change_addr)
    psbt = PSBT.from_base64(res["psbt"])
    assert len(psbt.tx.vout) == 1
    assert psbt.tx.vout[0].nValue > balance - 500
    sign_and_broadcast_psbt(coincubed, psbt)
    wait_for(
        lambda: all(
            c["spend_info"] is not None for c in coincubed.rpc.listcoins()["coins"]
        )
    )
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: 0.5,
    ...
    received_coins = coincubed.rpc.listcoins(["confirmed"])["coins"]
    ...
    destinations = {
        ...
        coincubed.rpc.getnewaddress()["address"]: int(0.3 * COIN),
    }
    res = coincubed.rpc.createspend(destinations, [spent_coin["outpoint"]], 1, change_addr)
    psbt = PSBT.from_base64(res["psbt"])
    assert len(psbt.tx.vout) == 3
    sign_and_broadcast_psbt(coincubed, psbt)
    wait_for(lambda: len(coincubed.rpc.listcoins(["unconfirmed"])["coins"]) == 1)
    wait_for(lambda: len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 2)
    balance = sum(
        c["amount"] for c in coincubed.rpc.listcoins(["unconfirmed", "confirmed"])["coins"]
    )
