...
def test_getinfo(coincubed):
    ...
    time.sleep(coincubed.poll_interval_secs + 1)
    res = coincubed.rpc.getinfo()
...
def test_update_derivation_indexes(coincubed):
    info = coincubed.rpc.getinfo()
    assert info["receive_index"] == 0
    assert info["change_index"] == 0

    ret = coincubed.rpc.updatederivationindexes(0, 0)
    info = coincubed.rpc.getinfo()
    assert info["receive_index"] == 0
    assert info["change_index"] == 0
    ...
    ret = coincubed.rpc.updatederivationindexes(receive=3)
    info = coincubed.rpc.getinfo()
    ...
    ret = coincubed.rpc.updatederivationindexes(change=4)
    info = coincubed.rpc.getinfo()
    assert info["receive_index"] == 3
    ...
    ret = coincubed.rpc.updatederivationindexes(receive=1, change=2)
    info = coincubed.rpc.getinfo()
    assert info["receive_index"] == 3
    assert info["change_index"] == 4
    assert ret["receive"] == 3
    ...
    ret = coincubed.rpc.updatederivationindexes(5, 6)
    info = coincubed.rpc.getinfo()
    ...
    with pytest.raises(
        ...
        coincubed.rpc.updatederivationindexes(-1)

    with pytest.raises(
        RpcError,
        match=re.escape("Invalid params: Invalid value for 'change' param"),
    ):
        coincubed.rpc.updatederivationindexes(0, -1)

    with pytest.raises(
        RpcError,
        match=re.escape("Unhardened or overflowing BIP32 derivation index."),
    ):
        coincubed.rpc.updatederivationindexes(MAX_DERIV + 1, 2)

    with pytest.raises(
        RpcError,
        match=re.escape("Unhardened or overflowing BIP32 derivation index."),
    ):
        coincubed.rpc.updatederivationindexes(0, MAX_DERIV + 1)

    with pytest.raises(
        RpcError,
        match=re.escape("Unhardened or overflowing BIP32 derivation index."),
    ):
        coincubed.rpc.updatederivationindexes(receive=(MAX_DERIV + 1))

    with pytest.raises(
        RpcError,
        match=re.escape("Unhardened or overflowing BIP32 derivation index."),
    ):
        coincubed.rpc.updatederivationindexes(change=(MAX_DERIV + 1))

    with pytest.raises(
        RpcError,
        match=re.escape("Invalid params: Missing 'receive' or 'change' parameter"),
    ):
        coincubed.rpc.updatederivationindexes()

    last_derivs = coincubed.rpc.updatederivationindexes(0, 0)
    last_receive = last_derivs["receive"]
    last_change = last_derivs["change"]

    ret = coincubed.rpc.updatederivationindexes(0, (MAX_DERIV - 1))
    ...
    ret = coincubed.rpc.updatederivationindexes((MAX_DERIV - 1), 0)
    assert ret["receive"] == last_receive + 1000
    assert ret["change"] == last_change


def test_getaddress(coincubed):
    res = coincubed.rpc.getnewaddress()
    assert "address" in res
    # The first new wallet address has index 1
    assert res["derivation_index"] == 1
    # We'll get a new one at every call
    assert res["address"] != coincubed.rpc.getnewaddress()["address"]
    # new address has derivation_index higher than the previous one
    assert coincubed.rpc.getnewaddress()["derivation_index"] == res["derivation_index"] + 2
    info = coincubed.rpc.getinfo()
    assert info["receive_index"] == res["derivation_index"] + 2  # 3 == 1 + 2
    assert info["change_index"] == 0


def test_listaddresses(coincubed):
    list1 = coincubed.rpc.listaddresses(2, 5)
    list2 = coincubed.rpc.listaddresses(start_index=2, count=5)
    assert list1 == list2
    ...
    with pytest.raises(
        ...
        coincubed.rpc.listaddresses("blabla", None)
    ...
    with pytest.raises(
        ...
        coincubed.rpc.listaddresses(0, "blb")


def test_listrevealedaddresses(coincubed, bitcoind):

    # Get addresses for reference:
    addresses = coincubed.rpc.listaddresses(0, 10)["addresses"]

    # We start with index 0 already "revealed":
    list_rec = coincubed.rpc.listrevealedaddresses(False, False, 10)
    ...
    addr_1 = coincubed.rpc.getnewaddress()["address"]
    addr_2 = coincubed.rpc.getnewaddress()["address"]
    addr_3 = coincubed.rpc.getnewaddress()["address"]
    addr_4 = coincubed.rpc.getnewaddress()["address"]
    addr_5 = coincubed.rpc.getnewaddress()["address"]
    addr_6 = coincubed.rpc.getnewaddress()["address"]
    addr_7 = coincubed.rpc.getnewaddress()["address"]

    # Last revealed receive index is 7.
    assert coincubed.rpc.getinfo()["receive_index"] == 7

    # Set some labels
    coincubed.rpc.updatelabels(
        {addr_1: "my test label 1", addr_5: "my test label 5"},
    )

    # Passing None or omitting start_index parameter is the same:
    assert coincubed.rpc.listrevealedaddresses(
        False, False, 10
    ) == coincubed.rpc.listrevealedaddresses(False, False, 10, None)

    # If we continue_from a value above our last revealed index, we'll start from the last index.
    assert coincubed.rpc.listrevealedaddresses(
        False, False, 10
    ) == coincubed.rpc.listrevealedaddresses(False, False, 10, 100)

    # Similarly if we start from a hardened index:
    assert coincubed.rpc.listrevealedaddresses(
        False, False, 10
    ) == coincubed.rpc.listrevealedaddresses(False, False, 10, 4_294_967_295)

    # Get 3 addresses starting at last revealed index:
    list_rec = coincubed.rpc.listrevealedaddresses(False, False, 3)
    ...
    list_rec = coincubed.rpc.listrevealedaddresses(False, False, 3, 4)
    ...
    list_rec = coincubed.rpc.listrevealedaddresses(False, False, 3, 1)
    ...
    list_rec = coincubed.rpc.listrevealedaddresses(False, True, 3)
    ...
    list_rec = coincubed.rpc.listrevealedaddresses(False, True, 3, 2)
    ...
    txid = bitcoind.rpc.sendmany("", destinations)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    wait_for(lambda: len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 5)
    ...
    txid = bitcoind.rpc.sendmany("", destinations)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    wait_for(lambda: len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 6)
    ...
    list_rec = coincubed.rpc.listrevealedaddresses(False, False, 0)
    ...
    assert coincubed.rpc.getinfo()["receive_index"] == 7
    assert coincubed.rpc.getinfo()["change_index"] == 0

    # We can get the single revealed change address:
    list_cha = coincubed.rpc.listrevealedaddresses(True, False, 3)
...
def test_listcoins(coincubed, bitcoind):
    ...
    assert len(coincubed.rpc.listcoins(["confirmed", "spent", "spending"])["coins"]) == 0
    assert (
        coincubed.rpc.listcoins()
        == coincubed.rpc.listcoins([], [outpoint_a])
        == coincubed.rpc.listcoins(["unconfirmed"])
        == coincubed.rpc.listcoins(["unconfirmed"], [outpoint_a])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spent", "unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spent", "unconfirmed", "confirmed"], [outpoint_a])
    )
    ...
    wait_for(lambda: coincubed.rpc.listcoins()["coins"][0]["block_height"] == block_height)

    assert (
        len(coincubed.rpc.listcoins())
        == len(coincubed.rpc.listcoins(["confirmed"])["coins"])
        == 1
    )
    assert (
        coincubed.rpc.listcoins()
        == coincubed.rpc.listcoins([], [outpoint_a])
        == coincubed.rpc.listcoins(["confirmed"])
        == coincubed.rpc.listcoins(["confirmed"], [outpoint_a])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spent", "unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spent", "unconfirmed", "confirmed"], [outpoint_a])
    )

    assert coincubed.rpc.listcoins()["coins"][0]["is_from_self"] is False

    # Same if the coin gets spent.
    spend_tx = spend_coins(coincubed, bitcoind, (res[0],))
    spend_txid = get_txid(spend_tx)
    wait_for(lambda: coincubed.rpc.listcoins()["coins"][0]["spend_info"] is not None)
    spend_info = coincubed.rpc.listcoins()["coins"][0]["spend_info"]
    assert spend_info["txid"] == spend_txid
    assert spend_info["height"] is None
    assert len(coincubed.rpc.listcoins(["spent"])["coins"]) == 0
    assert len(coincubed.rpc.listcoins(["spending"])["coins"]) == 1
    assert coincubed.rpc.listcoins(["spending"])["coins"][0]["is_from_self"] is False

    # And if this spending tx gets confirmed.
    bitcoind.generate_block(1, wait_for_mempool=spend_txid)
    curr_height = bitcoind.rpc.getblockcount()
    wait_for(lambda: coincubed.rpc.getinfo()["block_height"] == curr_height)
    spend_info = coincubed.rpc.listcoins()["coins"][0]["spend_info"]
    assert spend_info["txid"] == spend_txid
    assert spend_info["height"] == curr_height
    assert len(coincubed.rpc.listcoins(["unconfirmed", "confirmed"])["coins"]) == 0
    assert (
        coincubed.rpc.listcoins()
        == coincubed.rpc.listcoins(["spent"])
        == coincubed.rpc.listcoins(["spent", "unconfirmed", "confirmed"])
    )
    ...
    # Add a second coin.
    addr_b = coincubed.rpc.getnewaddress()["address"]
    txid_b = bitcoind.rpc.sendtoaddress(addr_b, 2)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 2)
    res = coincubed.rpc.listcoins(["unconfirmed"], [])["coins"]
    outpoint_b = res[0]["outpoint"]
    assert res[0]["is_from_self"] is False

    # We have one unconfirmed coin and one spent coin.
    assert (
        len(coincubed.rpc.listcoins()["coins"])
        == len(coincubed.rpc.listcoins([], [outpoint_a, outpoint_b])["coins"])
        == len(coincubed.rpc.listcoins(["unconfirmed", "spent"])["coins"])
        == len(
            coincubed.rpc.listcoins(["unconfirmed", "spent"], [outpoint_a, outpoint_b])[
                "coins"
            ]
        )
        == 2
    ...
    assert (
        coincubed.rpc.listcoins([], [outpoint_b])
        == coincubed.rpc.listcoins(["unconfirmed"])
        == coincubed.rpc.listcoins(["unconfirmed"], [outpoint_b])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spending", "unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["spending", "unconfirmed", "confirmed"], [outpoint_b])
    )
    assert coincubed.rpc.listcoins([], [outpoint_b])["coins"][0]["is_from_self"] is False

    # Now confirm the second coin.
    bitcoind.generate_block(1, wait_for_mempool=txid_b)
    block_height = bitcoind.rpc.getblockcount()
    wait_for(
        lambda: coincubed.rpc.listcoins([], [outpoint_b])["coins"][0]["block_height"]
        == block_height
    )
    assert coincubed.rpc.listcoins([], [outpoint_b])["coins"][0]["is_from_self"] is False
    ...
    assert (
        len(coincubed.rpc.listcoins()["coins"])
        == len(coincubed.rpc.listcoins([], [outpoint_a, outpoint_b])["coins"])
        == len(coincubed.rpc.listcoins(["confirmed", "spent"])["coins"])
        == len(
            coincubed.rpc.listcoins(["confirmed", "spent"], [outpoint_a, outpoint_b])[
                "coins"
            ]
        )
        == 2
    ...
    assert (
        coincubed.rpc.listcoins([], [outpoint_b])
        == coincubed.rpc.listcoins(["confirmed"])
        == coincubed.rpc.listcoins(["confirmed"], [outpoint_b])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed"])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed", "spending"])
        == coincubed.rpc.listcoins(["unconfirmed", "confirmed", "spending"], [outpoint_b])
    )

    # Add a third coin.
    addr_c = coincubed.rpc.getnewaddress()["address"]
    ...
    assert (
        len(coincubed.rpc.listcoins()["coins"])
        == len(coincubed.rpc.listcoins([], [outpoint_a, outpoint_b, outpoint_c])["coins"])
        == len(coincubed.rpc.listcoins(["unconfirmed", "confirmed", "spent"])["coins"])
        == len(
            coincubed.rpc.listcoins(
    ...
    assert (
        coincubed.rpc.listcoins([], [outpoint_c])
        == coincubed.rpc.listcoins(["unconfirmed"])
        == coincubed.rpc.listcoins(["unconfirmed"], [outpoint_c])
        == coincubed.rpc.listcoins(["unconfirmed", "spending"])
        == coincubed.rpc.listcoins(["spending", "unconfirmed"])
        == coincubed.rpc.listcoins(["spending", "unconfirmed", "confirmed"], [outpoint_c])
    )
    ...
    spend_txid = get_txid(spend_tx)
    wait_for(
        lambda: coincubed.rpc.listcoins([], [outpoint_c])["coins"][0]["spend_info"]
        is not None
    )

    assert len(coincubed.rpc.listcoins(["unconfirmed"])["coins"]) == 0
    assert (
        len(coincubed.rpc.listcoins()["coins"])
        == len(coincubed.rpc.listcoins([], [outpoint_a, outpoint_b, outpoint_c])["coins"])
        == len(coincubed.rpc.listcoins(["confirmed", "spending", "spent"])["coins"])
        == len(
            coincubed.rpc.listcoins(
    ...
    assert (
        coincubed.rpc.listcoins([], [outpoint_c])
        == coincubed.rpc.listcoins(["spending"])
        == coincubed.rpc.listcoins(["spending"], [outpoint_c])
        == coincubed.rpc.listcoins(["spending", "unconfirmed"])
        == coincubed.rpc.listcoins(["spending", "unconfirmed"], [outpoint_c])
    )

    # Add a fourth coin.
    addr_d = coincubed.rpc.getnewaddress()["address"]
    ...
    assert (
        len(coincubed.rpc.listcoins(["unconfirmed"])["coins"])
        == len(coincubed.rpc.listcoins(["confirmed"])["coins"])
        == len(coincubed.rpc.listcoins(["spending"])["coins"])
        == len(coincubed.rpc.listcoins(["spent"])["coins"])
        == 1
    )
    assert (
        len(coincubed.rpc.listcoins()["coins"])
        == len(
            coincubed.rpc.listcoins([], [outpoint_a, outpoint_b, outpoint_c, outpoint_d])[
                "coins"
            ]
        )
        == len(
            coincubed.rpc.listcoins(["unconfirmed", "confirmed", "spending", "spent"])[
                "coins"
            ]
        )
        == len(
    ...
    assert (
        sorted(
            coincubed.rpc.listcoins(["spending", "spent"])["coins"],
            key=lambda c: c["outpoint"],
        )
        == sorted(
            coincubed.rpc.listcoins(["spending", "spent"], [outpoint_a, outpoint_c])[
                "coins"
            ],
            key=lambda c: c["outpoint"],
        )
    ...
    for statuses, outpoints in [
        ...
        with pytest.raises(
            ...
            coincubed.rpc.listcoins(statuses, outpoints)
...
def test_jsonrpc_server(coincubed, bitcoind):
    """Test passing parameters as a list or a mapping."""
    addr = coincubed.rpc.getnewaddress()["address"]
    bitcoind.rpc.sendtoaddress(addr, 1)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 1)
    outpoints = [coincubed.rpc.listcoins()["coins"][0]["outpoint"]]
    destinations = {
        bitcoind.rpc.getnewaddress(): 20_000,
    }
    res = coincubed.rpc.createspend(destinations, outpoints, 18)
    assert "psbt" in res
...
def test_create_spend(coincubed, bitcoind):
    ...
    bitcoind.generate_block(1, wait_for_mempool=txid)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 16)
    ...
    assert coincubed.rpc.getinfo()["receive_index"] == 15
    ...
    imma_coin = next(c for c in coincubed.rpc.listcoins()["coins"] if c["is_immature"])
    with pytest.raises(RpcError, match=".*is from an immature coinbase transaction."):
        coincubed.rpc.createspend(destinations, [imma_coin["outpoint"]], 1)


def test_list_spend(coincubed, bitcoind):
    ...
    res = coincubed.rpc.createspend(destinations, outpoints, 6)
    assert "psbt" in res
    ...
    res_b = coincubed.rpc.createspend(destinations, outpoints, 2)
    assert "psbt" in res_b

    # Store them both in DB.
    time_before_update = int(time.time())
    assert len(coincubed.rpc.listspendtxs()["spend_txs"]) == 0
    coincubed.rpc.updatespend(res["psbt"])
    coincubed.rpc.updatespend(res_b["psbt"])

    # Check 'txids' parameter
    list_res = coincubed.rpc.listspendtxs()["spend_txs"]

    txid = PSBT.from_base64(list_res[0]["psbt"]).tx.txid().hex()

    filtered_res = coincubed.rpc.listspendtxs(txids=[txid])
    ...
    with pytest.raises(
        ...
        coincubed.rpc.listspendtxs(txids=[])

    with pytest.raises(RpcError, match="Invalid params: Invalid 'txids' parameter."):
        coincubed.rpc.listspendtxs(txids=[txid, 123])

    with pytest.raises(RpcError, match="Invalid params: Invalid 'txids' parameter."):
        coincubed.rpc.listspendtxs(txids=[0])

    with pytest.raises(RpcError, match="Invalid params: Invalid 'txids' parameter."):
        coincubed.rpc.listspendtxs(txids=[123])

    with pytest.raises(RpcError, match="Invalid params: Invalid 'txids' parameter."):
        coincubed.rpc.listspendtxs(txids=["abc"])
    ...
    coincubed.rpc.delspendtx(first_psbt.tx.txid().hex())
    list_res = coincubed.rpc.listspendtxs()["spend_txs"]
    ...
    coincubed.rpc.delspendtx(second_psbt.tx.txid().hex())
    list_res = coincubed.rpc.listspendtxs()["spend_txs"]
    assert len(list_res) == 0


def test_update_spend(coincubed, bitcoind):
    ...
    coincubed.rpc.updatespend(psbt_sig_a_ser)
    ...
    coincubed.rpc.updatespend(psbt_sig_b_ser)
...
def test_broadcast_spend(coincubed, bitcoind):
    ...
    with pytest.raises(RpcError, match="Failed to finalize the spend transaction.*"):
        coincubed.rpc.broadcastspend(txid)
    signed_psbt = coincubed.signer.sign_psbt(PSBT.from_base64(res["psbt"]))
...
def test_start_rescan_does_not_error(coincubed_with_deriv_paths, bitcoind):
    """Test we can successfully start a rescan."""
    tip_timestamp = bitcoind.rpc.getblockheader(bitcoind.rpc.getbestblockhash())["time"]
    coincubed_with_deriv_paths.rpc.startrescan(tip_timestamp - 1)


def test_start_rescan(coincubed, bitcoind):
    """Test we successfully retrieve all our transactions after losing state by rescanning."""
    initial_timestamp = int(time.time())
    first_address = coincubed.rpc.getnewaddress()
    second_address = coincubed.rpc.getnewaddress()

    # Some utility functions to DRY
    list_coins = lambda: coincubed.rpc.listcoins()["coins"]
    unspent_coins = lambda: (
        c for c in coincubed.rpc.listcoins()["coins"] if c["spend_info"] is None
    )
    ...
    tip_timestamp = bitcoind.rpc.getblockheader(bitcoind.rpc.getbestblockhash())["time"]
    coincubed.rpc.startrescan(tip_timestamp - 1)
    ...
    with pytest.raises(RpcError, match="Insane timestamp."):
        coincubed.rpc.startrescan(prebitcoin_timestamp)
    assert coincubed.rpc.getinfo()["rescan_progress"] is None
    # we can rescan from genesis block
    coincubed.rpc.startrescan(genesis_timestamp)
    wait_for(lambda: coincubed.rpc.getinfo()["rescan_progress"] is None)
    ...
    assert coincubed.rpc.getnewaddress() not in (first_address, second_address)
    ...
    # from 0.
    if BITCOIN_BACKEND_TYPE is BitcoinBackendType.Bitcoind:
        assert coincubed.rpc.getnewaddress() == first_address

    # Once the rescan is done, we must have detected all previous transactions.
    coincubed.rpc.startrescan(initial_timestamp)
    rescan_progress = coincubed.rpc.getinfo()["rescan_progress"]
...
def test_listtransactions(coincubed, bitcoind):
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: 0.0123456,
        coincubed.rpc.getnewaddress()["address"]: 0.0123457,
        coincubed.rpc.getnewaddress()["address"]: 0.0123458,
    }
    txid = bitcoind.rpc.sendmany("", destinations)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 3)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 5)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    outpoint = next(
        c["outpoint"] for c in coincubed.rpc.listcoins()["coins"] if txid in c["outpoint"]
    )
    destinations = {
        bitcoind.rpc.getnewaddress(): 100_000,
    }
    res = coincubed.rpc.createspend(destinations, [outpoint], 6)
    psbt = PSBT.from_base64(res["psbt"])
    txid = sign_and_broadcast_psbt(coincubed, psbt)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 7)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    outpoint = next(
        c["outpoint"] for c in coincubed.rpc.listcoins()["coins"] if txid in c["outpoint"]
    )
    destinations = {
        ...
        coincubed.rpc.getnewaddress()["address"]: 13_000,
    }
    res = coincubed.rpc.createspend(destinations, [outpoint], 6)
    psbt = PSBT.from_base64(res["psbt"])
    txid = sign_and_broadcast_psbt(coincubed, psbt)
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 11)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    outpoint = next(
        c["outpoint"] for c in coincubed.rpc.listcoins()["coins"] if txid in c["outpoint"]
    )
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 12)
    ...
    txids = set(c["outpoint"][:-2] for c in coincubed.rpc.listcoins()["coins"])
    assert len(txids) == 8
    ...
    all_txs = coincubed.rpc.listtransactions(list(txids))["transactions"]
    ...
    txids = set(c["outpoint"][:-2] for c in coincubed.rpc.listcoins()["coins"])
    for txid in txids:
        txs = coincubed.rpc.listtransactions([txid])["transactions"]
    ...
    txs = coincubed.rpc.listconfirmed(initial_timestamp, final_timestamp, 10)[
        "transactions"
    ]
    ...
    txs = coincubed.rpc.listconfirmed(initial_timestamp, final_timestamp, 5)[
        "transactions"
    ]
    assert len(txs) == 5
    ...
    txids = set()
    for coin in coincubed.rpc.listcoins()["coins"]:
    ...
    assert len(txids) == 3
    # Now let's compare with what coincubed is giving us.
    txs = coincubed.rpc.listconfirmed(second_timestamp, third_timestamp, 10)[
        "transactions"
    ]
    assert len(txs) == 3
...
def test_create_recovery(coincubed, bitcoind):
    ...
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: 0.1,
        coincubed.rpc.getnewaddress()["address"]: 0.2,
        coincubed.rpc.getnewaddress()["address"]: 0.3,
    }
    txid = bitcoind.rpc.sendmany("", destinations)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    wait_for(
    ...
    first_outpoints = [c["outpoint"] for c in coincubed.rpc.listcoins()["coins"]]
    ...
    with pytest.raises(
        ...
        coincubed.rpc.createrecovery(bitcoind.rpc.getnewaddress(), 2)
    # Same if we specify timelock:
    with pytest.raises(
        RpcError,
        match="No coin currently spendable through this timelocked recovery path",
    ):
        coincubed.rpc.createrecovery(bitcoind.rpc.getnewaddress(), 2, 10)
    ...
    res = coincubed.rpc.createrecovery(reco_address, 18)
    reco_psbt = PSBT.from_base64(res["psbt"])

    # Do the same passing all three coins explicitly:
    res_op = coincubed.rpc.createrecovery(reco_address, 18, 10, first_outpoints)
    ...
    res_op_2 = coincubed.rpc.createrecovery(
    ...
    txid = sign_and_broadcast(coincubed, bitcoind, reco_psbt, recovery=True)
    ...
    res = coincubed.rpc.createrecovery(bitcoind.rpc.getnewaddress(), 1)
    reco_psbt = PSBT.from_base64(res["psbt"])
...
def test_labels(coincubed, bitcoind):
    """Test the creation and updating of labels."""
    # We can set a label for an address.
    addr = coincubed.rpc.getnewaddress()["address"]
    coincubed.rpc.updatelabels({addr: "first-addr"})
    assert coincubed.rpc.getlabels([addr])["labels"] == {addr: "first-addr"}
    # And also update it.
    coincubed.rpc.updatelabels({addr: "first-addr-1"})
    assert coincubed.rpc.getlabels([addr])["labels"] == {addr: "first-addr-1"}
    ...
    with pytest.raises(RpcError, match=".*must be less or equal than 100 characters"):
        coincubed.rpc.updatelabels({addr: "".join("a" for _ in range(101))})

    # We can set a label for a coin.
    sec_addr = coincubed.rpc.getnewaddress()["address"]
    txid = bitcoind.rpc.sendtoaddress(sec_addr, 1)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 1)
    coin = coincubed.rpc.listcoins()["coins"][0]
    coincubed.rpc.updatelabels({coin["outpoint"]: "first-coin"})
    assert coincubed.rpc.getlabels([coin["outpoint"]])["labels"] == {
        coin["outpoint"]: "first-coin"
    }
    # And also update it.
    coincubed.rpc.updatelabels({coin["outpoint"]: "first-coin-1"})
    assert coincubed.rpc.getlabels([coin["outpoint"]])["labels"] == {
        coin["outpoint"]: "first-coin-1"
    }
    # Its address though has no label.
    assert coincubed.rpc.getlabels([sec_addr])["labels"] == {}
    # But we can receive a coin to the address that has a label set, and query both.
    sec_txid = bitcoind.rpc.sendtoaddress(addr, 1)
    wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == 2)
    sec_coin = next(
        c for c in coincubed.rpc.listcoins()["coins"] if sec_txid in c["outpoint"]
    )
    coincubed.rpc.updatelabels({sec_coin["outpoint"]: "sec-coin"})
    res = coincubed.rpc.getlabels([sec_coin["outpoint"], addr])["labels"]
    assert len(res) == 2
    assert res[sec_coin["outpoint"]] == "sec-coin"
    assert res[addr] == "first-addr-1"
    # We can also query the labels for both coins, of course.
    res = coincubed.rpc.getlabels([coin["outpoint"], sec_coin["outpoint"]])["labels"]
    assert len(res) == 2
    assert res[coin["outpoint"]] == "first-coin-1"
    assert res[sec_coin["outpoint"]] == "sec-coin"

    # We can set, update and query labels for deposit transactions.
    coincubed.rpc.updatelabels({txid: "first-deposit"})
    assert coincubed.rpc.getlabels([txid, sec_txid])["labels"] == {txid: "first-deposit"}
    coincubed.rpc.updatelabels({txid: "first-deposit-1", sec_txid: "second-deposit"})
    res = coincubed.rpc.getlabels([txid, sec_txid])["labels"]
    assert len(res) == 2
    assert res[txid] == "first-deposit-1"
    assert res[sec_txid] == "second-deposit"

    # We can set and update a label for a spend transaction.
    spend_txid = get_txid(spend_coins(coincubed, bitcoind, [coin, sec_coin]))
    coincubed.rpc.updatelabels({spend_txid: "spend-tx"})
    assert coincubed.rpc.getlabels([spend_txid])["labels"] == {spend_txid: "spend-tx"}
    coincubed.rpc.updatelabels({spend_txid: "spend-tx-1"})
    assert coincubed.rpc.getlabels([spend_txid])["labels"] == {spend_txid: "spend-tx-1"}
    ...
    res = coincubed.rpc.getlabels([inexistent_txid, inexistent_outpoint, random_address])[
        "labels"
    ]
    assert len(res) == 3
    ...
    bitcoind.generate_block(1, wait_for_mempool=spend_txid)
    wait_for(
        lambda: bitcoind.rpc.getblockcount() == coincubed.rpc.getinfo()["block_height"]
    )
    res = coincubed.rpc.getlabels(
        [
            addr,
    ...
    res = coincubed.rpc.getlabels([addr, sec_addr, random_address])["labels"]
    assert len(res) == 1
...
def test_labels_bip329(coincubed, bitcoind):
    # Label 5 addresses
    addresses = []
    for i in range(0, 5):
        addr = coincubed.rpc.getnewaddress()["address"]
        addresses.append(addr)
        coincubed.rpc.updatelabels({addr: f"addr{i}"})
    ...
    for i in range(0, 5):
        ...
        txids.append(txid)
        wait_for(lambda: len(coincubed.rpc.listcoins()["coins"]) == i + 1)

    coins = coincubed.rpc.listcoins()["coins"]
    for i in range(0, 5):
        coin = coins[i]
        coincubed.rpc.updatelabels({coin["outpoint"]: f"coin{i}"})

    # Label 5 transactions
    for i, txid in enumerate(txids):
        coincubed.rpc.updatelabels({txid: f"tx{i}"})

    # Get Bip-0329 labels
    bip329_labels = coincubed.rpc.getlabelsbip329(0, 100)["labels"]
    ...
    batch1 = coincubed.rpc.getlabelsbip329(0, 5)["labels"]
    assert len(batch1) == 5

    batch2 = coincubed.rpc.getlabelsbip329(5, 5)["labels"]
    assert len(batch2) == 5

    batch3 = coincubed.rpc.getlabelsbip329(10, 5)["labels"]
    assert len(batch3) == 5
...
def test_rbfpsbt_bump_fee(coincubed, bitcoind):
    ...
    # Get three coins.
    destinations = {
        coincubed.rpc.getnewaddress()["address"]: 0.003,
        coincubed.rpc.getnewaddress()["address"]: 0.004,
        coincubed.rpc.getnewaddress()["address"]: 0.005,
    }
    txid = bitcoind.rpc.sendmany("", destinations)
    bitcoind.generate_block(1, wait_for_mempool=txid)
    wait_for(lambda: len(coincubed.rpc.listcoins(["confirmed"])["coins"]) == 3)
    ...
    for bad_feerate in [-1, "foo", 18_446_744_073_709_551_616]:
        with pytest.raises(RpcError, match=f"Invalid 'feerate' parameter."):
            coincubed.rpc.rbfpsbt(first_txid, False, bad_feerate)
    ...
    with pytest.raises(RpcError, match=f"Unknown spend transaction '{first_txid}'."):
        coincubed.rpc.rbfpsbt(first_txid, False, 1)
    # Now save the PSBT.
    coincubed.rpc.updatespend(first_res["psbt"])
    # The RBF command succeeds even if transaction has not been signed.
    coincubed.rpc.rbfpsbt(first_txid, False, 2)
    # The RBF command also succeeds if transaction has been signed but not broadcast.
    first_psbt = coincubed.signer.sign_psbt(first_psbt)
    coincubed.rpc.updatespend(first_psbt.to_base64())
    coincubed.rpc.rbfpsbt(first_txid, False, 2)
    # Now broadcast the spend and wait for it to be detected.
    coincubed.rpc.broadcastspend(first_txid)
    wait_for(
        lambda: all(
    ...
    lc_res = coincubed.rpc.listcoins(["unconfirmed"], [])["coins"]
    assert (
        len(lc_res) == 1
    ...
    coincubed.rpc.delspendtx(first_txid)
    coincubed.rpc.rbfpsbt(first_txid, False, 2)

    # Let's use an even higher feerate.
    rbf_1_res = coincubed.rpc.rbfpsbt(first_txid, False, 10)
    ...
    rbf_1_txid = sign_and_broadcast_psbt(coincubed, rbf_1_psbt)
    wait_for(
        lambda: all(
    ...
    with pytest.raises(
        ...
        coincubed.rpc.rbfpsbt(first_txid, False, int(rbf_1_feerate))
    # Using 1 more for feerate works.
    feerate = int(rbf_1_feerate) + 1
    coincubed.rpc.rbfpsbt(first_txid, False, feerate)
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins([], desc_1_outpoints)["coins"]) == 2)
    desc_1_res = coincubed.rpc.createspend(desc_1_destinations, desc_1_outpoints, 1)
    desc_1_psbt = PSBT.from_base64(desc_1_res["psbt"])
    assert len(desc_1_psbt.tx.vout) == 2
    desc_1_txid = sign_and_broadcast_psbt(coincubed, desc_1_psbt)
    wait_for(
        lambda: all(
            c["spend_info"] is not None and c["spend_info"]["txid"] == desc_1_txid
            for c in coincubed.rpc.listcoins([], desc_1_outpoints)["coins"]
        )
    )
    lc_res = [c for c in coincubed.rpc.listcoins(["unconfirmed"], [])["coins"]]
    assert (
        len(lc_res) == 1
    ...
    wait_for(lambda: len(coincubed.rpc.listcoins([], desc_2_outpoints)["coins"]) == 1)
    desc_2_res = coincubed.rpc.createspend(desc_2_destinations, desc_2_outpoints, 1)
    desc_2_psbt = PSBT.from_base64(desc_2_res["psbt"])
    assert len(desc_2_psbt.tx.vout) == 2
    desc_2_txid = sign_and_broadcast_psbt(coincubed, desc_2_psbt)
    wait_for(
        lambda: all(
            c["spend_info"] is not None and c["spend_info"]["txid"] == desc_2_txid
            for c in coincubed.rpc.listcoins([], desc_2_outpoints)["coins"]
        )
    )
    lc_res = [c for c in coincubed.rpc.listcoins(["unconfirmed"], [])["coins"]]
    assert (
    ...
    rbf_2_res = coincubed.rpc.rbfpsbt(rbf_1_txid, False, feerate)
    ...
    # Broadcast the replacement and wait for it to be detected.
    rbf_2_txid = sign_and_broadcast_psbt(coincubed, rbf_2_psbt)
    wait_for(
        lambda: all(
    ...
    assert coincubed.rpc.listcoins([], desc_1_outpoints + desc_2_outpoints)["coins"] == [
        coins[2]
    ]
    ...
    final_coins = coincubed.rpc.listcoins()["coins"]
...
def test_rbfpsbt_cancel(coincubed, bitcoind):
    ...
    wait_for(
        lambda: all(
            c["spend_info"] is not None and c["spend_info"]["txid"] == rbf_1_txid
            for c in coincubed.rpc.listcoins([], [rbf_1_outpoint])["coins"]
        )
    )
    ...
    wait_for(
        lambda: all(
            ...
            for c in coincubed.rpc.listcoins(
    ...
    rbf_2_res = coincubed.rpc.rbfpsbt(rbf_1_txid, True)
    rbf_2_psbt = PSBT.from_base64(rbf_2_res["psbt"])
    # The inputs are the same in both (no new inputs needed in the replacement).
